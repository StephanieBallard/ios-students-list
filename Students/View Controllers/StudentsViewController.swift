...
class StudentsViewController: UIViewController {
    ...
    private var filteredAndSortedStudents: [Student] = [] {
        didSet { //whenever the array in this computed property changes the tableview will be told to reload it's data
        ...
        studentController.loadFromPersistentStore { (students, error) in
            ...
            DispatchQueue.main.async {
                if let students = students {
                    self.filteredAndSortedStudents = students //triggers code that updates the UI
                }
            }
        }
    }
